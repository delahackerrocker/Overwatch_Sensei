...
public class SigmaHD : HeroData
{
    public SigmaHD() : base()
    {
        ...
        abilities[aCount].abilityDetails[++bCount] = "\nIf Melee is used correctly after the second projectile, the time until the next attack will not be increased.";
        abilities[aCount].abilityDetails[++bCount] = "\nBounces off walls, floors, and ceilings.";
        abilities[aCount].abilityDetails[++bCount] = "\nEnemies hurt by the implosion will be slightly pulled towards the center.";
        abilities[aCount].abilityDetails[++bCount] = "\nThe charges automatically implode after travelling 22 meters.";
        ...
        abilities[aCount].abilityDetails[++bCount] = "<b>Cooldown:</b> 2 seconds when recalled\n          5 seconds if destroyed";
        ...
        abilities[aCount].abilityDetails[++bCount] = "<b>Duration:</b> 0.8 seconds knock down";
        abilities[aCount].abilityDetails[++bCount] = "<b>Cooldown:</b> 10 seconds";
        abilities[aCount].abilityDetails[++bCount] = "\nThe stone throw animation is 0.640 seconds.";
        abilities[aCount].abilityDetails[++bCount] = "\nThe stone can visually touch the wall, but not explode and fly further. Hitbox for wall explosions is smaller.";
        abilities[aCount].abilityDetails[++bCount] = "\nIgnores Deflect, Defense Matrix, and Kinetic Grasp.";
        ...
        abilities[aCount].abilityDetails[bCount = 0] = "<b>Health:</b> 60% of damage absorbed is converted to temporary shields (max. 400)";
        abilities[aCount].abilityDetails[++bCount] = "<b>Max Range:</b> 3 meters";
        abilities[aCount].abilityDetails[++bCount] = "<b>Duration:</b> 2 seconds + 0.3 s. recovery (gain shields)";
        ...
        abilities[aCount].abilityDetails[++bCount] = "\nSome abilities are given assigned values due their unorthodox nature. For example, the damage Biotic Orb is credited as 100 damage.";
        abilities[aCount].abilityDetails[++bCount] = "\nCan be cancelled with Accretion or ult. However, no shields will be gained if Kinetic Grasp is cancelled early.";
        ...
        abilities[aCount].abilityDetails[++bCount] = "<b>Duration:</b> Up to 5 seconds to select the area\n          1 second(from confirm to suspend)\n          Then after 2 seconds slam starts\n          After slam starts 0.6 s.high gravity";
        abilities[aCount].abilityDetails[++bCount] = "<b>Ultimate Cost:</b> 1960 points";
        abilities[aCount].abilityDetails[++bCount] = "\nSigma passively generates 1% Ult Charge every 3.92 seconds.";
        abilities[aCount].abilityDetails[++bCount] = "\nHolding the Jump key causes Sigma to rise, while holding Crouch makes him descend.";
        abilities[aCount].abilityDetails[++bCount] = "\nDoes not affect deployed objects such as turrets (but it does move Bastion in Sentry mode).";
        abilities[aCount].abilityDetails[++bCount] = "\nDisables all movement abilities of enemies suspended in the air, except Moira's Fade and Reaper's Wraith Form.";
        abilities[aCount].abilityDetails[++bCount] = "\nIf the ability is interrupted before the slam, then there will be no slam and the enemies will no longer be under the influence of the ultimate.";
