...
public class ReinhardtHD : HeroData
{
    public ReinhardtHD() : base()
    {
        ...
        abilities[aCount].abilityDetails[bCount = 0] = "Damage:85";
        abilities[aCount].abilityDetails[++bCount] = "Max. range:5 meters";
        abilities[aCount].abilityDetails[++bCount] = "Rate of fire:1 swing per 0.94 second";
        abilities[aCount].abilityDetails[++bCount] = "Casting time:0.416 second (initial swing)\n          0.208 second(dealing damage)\n          0.320 second(recovery after dealing damage ends)";
        ...
        abilities[aCount].abilityDetails[bCount = 0] = "Health:1600";
        abilities[aCount].abilityDetails[++bCount] = "Healing:Regenerates 200 barrier health per second, after being down for 2 seconds";
        abilities[aCount].abilityDetails[++bCount] = "Move. speed:-30% penalty";
        abilities[aCount].abilityDetails[++bCount] = "Area of effect:Height 4.250 meters. Width 6.800 meters.";
        abilities[aCount].abilityDetails[++bCount] = "Casting time:0.08 seconds";
        abilities[aCount].abilityDetails[++bCount] = "Cooldown:5 seconds if destroyed";
        abilities[aCount].abilityDetails[++bCount] = "By holding the primary fire key, Reinhardt can look around at his surroundings without turning.";
        abilities[aCount].abilityDetails[++bCount] = "The barrier cracks once at 50% and once at 25%. At 20%, Reinhardt will say his barrier is failing.";
        ...
        abilities[aCount].abilityDetails[bCount = 0] = "Damage:Pinned to a wall: 300\n        Bump with no pin: 50";
        abilities[aCount].abilityDetails[++bCount] = "Move. speed:+200% buff";
        abilities[aCount].abilityDetails[++bCount] = "Max. range:49.5 meters";
        abilities[aCount].abilityDetails[++bCount] = "Casting time:0.6 seconds + 0.5 s. recovery if pin the wall";
        ...
        abilities[aCount].abilityDetails[++bCount] = "Reinhardt can turn while charging but his control is reduced.";
        abilities[aCount].abilityDetails[++bCount] = "If Charge directly collides with an enemy, that enemy is 'pinned' (stunned), and if they hit a wall, the pinned target takes 300 damage. If Charge ends before hitting a wall, no damage is inflicted, but the target will remain stunned for an additional 0.65 seconds. Only 1 enemy can be pinned at a time.";
        abilities[aCount].abilityDetails[++bCount] = "If Charge indirectly collides with an enemy, or if there is already a target pinned, the enemy is knocked away and takes 50 damage.";
        abilities[aCount].abilityDetails[++bCount] = "If Charge collides with Rocket Punch, Shield Bash, a charging B.O.B., or another Charge, both participants get knocked down for 2 seconds. Any pinned targets will be damaged as if they hit a wall.";
        ...
        abilities[aCount].abilityDetails = new string[8];
        abilities[aCount].abilityDetails[bCount = 0] = "Damage:100";
        abilities[aCount].abilityDetails[++bCount] = "Projectile speed:25 meters per second";
        abilities[aCount].abilityDetails[++bCount] = "Casting time:0.496 seconds for throw + 0.620 s. recovery";
        abilities[aCount].abilityDetails[++bCount] = "Cooldown:6 seconds";
        abilities[aCount].abilityDetails[++bCount] = "Headshot:?";
        abilities[aCount].abilityDetails[++bCount] = "Pierces barriers, enemies, and other damageable entities.";
        abilities[aCount].abilityDetails[++bCount] = "This ability can animation cancel Ability-reinhardt1.png Rocket Hammer, allowing for faster attacks.";
        abilities[aCount].abilityDetails[++bCount] = "If the center of the projectile does not touch the wall, then the projectile will not explode against the wall.";
        ...
        abilities[aCount].abilityDetails[++bCount] = "Area of effect:height 1.770 meters";
        abilities[aCount].abilityDetails[++bCount] = "Casting time:0.45 seconds + 0.5 s. recovery";
        abilities[aCount].abilityDetails[++bCount] = "Duration:2.5 second knock down";
        abilities[aCount].abilityDetails[++bCount] = "Ultimate cost:1540 points";
        abilities[aCount].abilityDetails[++bCount] = "Reinhardt passively generates 1% Ult Charge every 3.08 seconds.";
        abilities[aCount].abilityDetails[++bCount] = "Requires line of sight.";
        abilities[aCount].abilityDetails[++bCount] = "Earthshatter has a small amount of vertical height (1.770 meters), allowing it to affect enemies in the air.";
        abilities[aCount].abilityDetails[++bCount] = "Earthshatter Will be stopped by barriers, even if the damage from Earthshatter breaks the barrier.";
        abilities[aCount].abilityDetails[++bCount] = "Earthshatter deals an additional 200 damage within 1.75m of the impact area.";
        ...
        abilities[aCount].abilityDetails[++bCount] = "The distance in which Reinhardt is knocked back is NOT reduced by 30%; it is reduced by more. This is because the velocity has to accelerate back to normal; a higher initial velocity means more time at higher velocities and more time with a velocity in general.";
        abilities[aCount].abilityDetails[++bCount] = "Does not affect Halt!, Chain Hook, Graviton Surge, or Gravitic Flux because they are not knock back effects.";
